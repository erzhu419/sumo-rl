...
class SAC_Trainer():
    ...
    def load_model(self, path):
        self.soft_q_net.load_state_dict(torch.load(path + '_q', weights_only=True, map_location="cpu"))
        self.policy_net.load_state_dict(torch.load(path + '_policy', weights_only=True, map_location="cpu"))
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.resume_checkpoint:
            ...
            # Try to load existing reward/monitoring logs to prevent overwriting history
            try:
                log_files = {
                    'rewards.npy': rewards,
                    'q_values_episode.npy': q_values_episode,
                    'reg_norms1_episode.npy': reg_norms1_episode,
                    'reg_norms2_episode.npy': reg_norms2_episode,
                    'log_probs_episode.npy': log_probs_episode,
                    'alpha_values_episode.npy': alpha_values_episode,
                    'ood_losses_episode.npy': ood_losses_episode,
                    'q_stds_episode.npy': q_stds_episode
                }
                for filename, target_list in log_files.items():
                    log_path = os.path.join(LOG_DIR, filename)
                    if os.path.exists(log_path):
                        data = np.load(log_path)
                        target_list.extend(list(data))
                print(f"Loaded existing monitoring logs from {LOG_DIR}")
            except Exception as e:
                print(f"Warning: Could not load some monitoring logs: {e}")
            ...
            try:
                ...
                if 'episode_' in base_name:
                    ...
                    # If we loaded logs, ensure start_episode matches log length to be safe
                    if len(rewards) > 0 and len(rewards) != start_episode:
                        print(f"Note: Log length ({len(rewards)}) differs from inferred start episode ({start_episode}). Using start_episode={start_episode}")
            except Exception as e:
                print(f"Could not infer start episode from {args.resume_checkpoint}. Starting from 0 but keeping weights.")

        args.train = False
        # When only evaluating specific episodes (not range(49)),
        # we must advance step past warmup to match the original behavior
        # where step accumulated across all prior episodes.
        step = max(step, args.warmup_steps)
        model_dir = "model/sac_ensemble_SUMO_linear_penalty_Production_Augmented_BangBang_V7_Long"
        recovered_rewards = []
        for eps in [39]:
            ckpt = os.path.join(model_dir, f"checkpoint_episode_{eps}")
            if not os.path.exists(ckpt + "_policy"):
                print(f"Skipping {eps}")
            ...
            while not done:
                        ...
                        if len(history) == 1:
                            if action_dict[line_id][bus_id] is None:
                                ...
                                if action_val is None:
                                    action_str = "0.0"
                                elif isinstance(action_val, (np.ndarray, list)):
                                    action_str = "_".join([f"{v:.4f}" for v in action_val])
                                else:
                                    action_str = f"{action_val:.4f}"
                ...
                if False and len(replay_buffer) > args.batch_size and len(replay_buffer) % args.training_freq == 0 and step_trained != step and step >= args.warmup_steps:
                    step_trained = step
        ...
        print("Done recovery! Max:", np.max(recovered_rewards), "at index", np.argmax(recovered_rewards))
