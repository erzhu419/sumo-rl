...
def get_smooth_ridge_reward(fh, bh):
    dev_f = abs(fh - t_f)
    dev_b = abs(bh - t_b)
    ...
    # User's tanh approach
    penalty_factor = 20 * np.tanh((abs(fh - t_f) - 0.5 * t_f) / 30) + 20 * np.tanh((abs(bh - t_b) - 0.5 * t_b) / 30)
    reward -= max(0, penalty_factor)
...
for i in range(X.shape[0]):
    for j in range(X.shape[1]):
        Z[i, j] = get_smooth_ridge_reward(X[i, j], Y[i, j])
...
layout = go.Layout(
    title=f'Smoothed Reward Surface Tanh (Target {t_f}:{t_b})',
    scene=dict(
...
fig.write_html("reward_surface_smooth.html")
print("Image saved to reward_surface_smooth.html")
