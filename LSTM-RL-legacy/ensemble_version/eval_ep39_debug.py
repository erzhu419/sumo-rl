...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        for eps in range(1):
            sac_trainer.load_model(os.path.join(SAVE_ROOT, 'model', 'sac_ensemble_SUMO_linear_penalty_Production_Augmented_BangBang_V7_Long', 'checkpoint_episode_39'))
            print("DEBUG: Loaded checkpoint_episode_39. Starting episode loop...", flush=True)
            episode_start_time = time.time()
            if eps != 0:
                env.reset()
            print(f"DEBUG: init_state done={_}", flush=True)
            state_dict, reward_dict, _ = safe_initialize_state(env, render=render)

            print(f"DEBUG: episode_steps={episode_steps}, done={done}", flush=True)
            done = False
            episode_steps = 0
            training_steps = 0
            ...
            if eps % args.plot_freq == 0:
                ...
                pass # sac_trainer.save_model(os.path.join(MODEL_DIR, f"checkpoint_episode_{eps}"))
        ...
        pass # sac_trainer.save_model(os.path.join(MODEL_DIR, "final"))
