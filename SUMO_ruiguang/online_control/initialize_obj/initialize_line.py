...
def initialize_line_obj(net_path_ex, stop_path_ex, bus_rou_path_ex, detector_output_path_1_ex, detector_output_path_2_ex):
    ...
    for bus_line in bus_line_list:  # LRG 针对每条线路，计算line类所需要的静态信息
        for child in bus_rou_root:
            if_jump_loop = "no"
            for sub_child in child:
                if sub_child.tag == "stop" and sub_child.get("line") == bus_line:
                    ...
                    for edge_id in edge_id_list:  # LRG 分别记录每个路段在每个小时内的运行速度，作为一个列表，然后取平均作为该路段每个小时的平均速度 [第1小时速度,第2小时速度,...]
                        speed_list_every_hour = [[], [], [], [], []]  # LRG 时刻表不会超过5个小时，这里以小时为一个[]，故一共有5个[]
...
line_obj_dic = initialize_line_obj("../../b_network/5g_changsha_bus_network_with_signal_d.net.xml",
                                   "../../b_network/3_bus_station.add.xml",
                                   "../../d_bus_rou/2_bus_timetable.rou.xml",
                                   "../output_file/1_output_signal_sensor_info.xml",
                                   "../output_file/1_output_edge_sensor_info.xml")
# 创建XML根元素
